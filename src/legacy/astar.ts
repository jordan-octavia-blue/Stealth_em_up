// @ts-nocheck -- vendored third-party file, kept verbatim; not ours to type.
...
// The upstream file ships a UMD preamble that sniffs for CommonJS/AMD before falling
// back to `window`. That sniffing is actively harmful now the file is bundled: seeing
// `module.exports` in the source makes Vite classify the whole chunk as CommonJS, hand it
// a real `module` object, and the library registers itself into a throwaway export that
// nothing reads — `astar` and `Graph` never reach `window` and the game dies on the first
// path request. The browser branch is the only one this project ever wanted, so it is now
// the only branch. Everything below the preamble is upstream, untouched.
(function(definition) {
...
// (no top-level function declarations in this file)
